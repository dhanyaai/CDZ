...
import { CheckCircle2, Clock, AlertCircle, CalendarClock, Plus, Building2, Target, Trash2 } from "lucide-react";
    ...
    <div className="flex items-start gap-3 p-3 rounded-lg border bg-card hover:bg-card/80 transition-colors">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline" className={`text-[10px] px-1.5 py-0 border ${TYPE_COLORS[item.type] ?? TYPE_COLORS.task}`}>
            {item.type}
          </Badge>
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground flex-wrap">
          {item.clientName && (
            <span className="flex items-center gap-1"><Building2 className="w-3 h-3" />{item.clientName}</span>
          )}
          {item.leadTitle && (
            <span className="flex items-center gap-1"><Target className="w-3 h-3" />{item.leadTitle}</span>
          )}
          {item.dueDate && (
...
function Section({ title, icon: Icon, items, color, onComplete, onDelete }: {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <FollowUpRow key={item.id} item={item} onComplete={onComplete} onDelete={onDelete} />
        ))}
      </div>
    </div>
  );
...
export function FollowUps() {
  ...
  const [form, setForm] = useState({ type: "follow-up", subject: "", description: "", dueDate: "" });

  const { data: pending, isLoading: loadingPending } = useQuery({
    queryKey: ["followups", "pending"],
    queryFn: () => api<FollowUp[]>("/v1/activities?pending=true"),
  });

  const { data: completed, isLoading: loadingCompleted } = useQuery({
    queryKey: ["followups", "completed"],
    queryFn: () => api<FollowUp[]>("/v1/activities"),
    enabled: showCompleted,
    select: (data) => data.filter((a) => !!a.completedAt),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowDialog(false);
      setForm({ type: "follow-up", subject: "", description: "", dueDate: "" });
          ...
          <div className="space-y-3 pt-1">
            <div className="space-y-1.5">
              ...
              <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}>
                ...
                <SelectContent>
                  {["follow-up", "call", "email", "meeting", "task"].map((t) => (
            ...
            <Button
              ...
              disabled={!form.subject.trim() || createMutation.isPending}
            >
              Schedule
            </Button>
          </div>
