...
const STATUS_META: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  Received:   { label: "Accepted",   color: "bg-emerald-500/10 text-emerald-600 border-emerald-500/20", icon: CheckCircle2 },
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-14">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">
                          {tab === "All" ? 'Click "New Sample Order" to create one' : `No orders with status "${sampleStatusLabel(tab)}"`}
                        </p>
                      </TableCell>
                    </TableRow>
                  ) : (
