...
import { Plus, Trash2, Calendar, Building2, IndianRupee, TrendingUp, Target, BarChart3 } from "lucide-react";
...
export function Opportunities() {
  ...
  const [form, setForm] = useState({ title: "", clientId: "", value: "", probability: "50", expectedCloseDate: "", notes: "" });
  ...
  const create = useMutation({
    mutationFn: () => api("/v1/opportunities", { method: "POST", body: JSON.stringify({
      ...
      expectedCloseDate: form.expectedCloseDate || null, notes: form.notes || null,
    })}),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["opportunities"] }); setDialog(false);
      setForm({ title: "", clientId: "", value: "", probability: "50", expectedCloseDate: "", notes: "" });
              ...
              <div className="flex-1 p-2 space-y-2 overflow-y-auto">
                {items.map(o => {
                  ...
                  return (
                    <div
                      key={o.id}
                      className="p-2.5 border rounded-lg bg-card text-xs space-y-2 cursor-pointer hover:border-primary/40 hover:shadow-sm transition-all"
                      onClick={() => setSelected(o)}
                    >
                      <div className="font-medium leading-tight">{o.title}</div>
                      {o.clientName && (
                        <div className="flex items-center gap-1 text-muted-foreground">
                          <Building2 className="w-3 h-3 shrink-0" />{o.clientName}
                        </div>
                      )}
                      {o.value != null && (
                        <div className="font-semibold text-primary">₹{o.value.toLocaleString("en-IN", { maximumFractionDigits: 0 })}</div>
                      )}
                ...
                {items.length === 0 && (
                  <div className="text-center text-muted-foreground/50 text-xs py-6">No opportunities</div>
                )}
              </div>
            </div>
          );
            ...
            <>
              <SheetHeader className="mb-6">
                ...
                {selected.clientName && (
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <Building2 className="w-4 h-4" />{selected.clientName}
                  </p>
                )}
              </SheetHeader>
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={selected.stage} onValueChange={v => {
                    update.mutate({ id: selected.id, data: { stage: v } });
