...
function serializeProduct(p: typeof productsTable.$inferSelect, vendorName?: string | null) {
  return {
    ...
    branding: p.branding != null ? Number(p.branding) : null,
    transportation: p.transportation != null ? Number(p.transportation) : null,
...
router.post("/v1/products", async (req, res): Promise<void> => {
  ...
  const [product] = await db
    ...
    .values({
      ...
      branding: branding != null ? String(branding) : null,
      transportation: transportation != null ? String(transportation) : null,
